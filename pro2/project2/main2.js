...
function getTaskFromStorage(){
  let retrivedTask=JSON.parse(localStorage.getItem("tasksss")) ;
  tasksss= retrivedTask ?? []
...
document.getElementById('add-button').addEventListener("click", function () {
 ...
 tasksss.push(taskObj);
 storeTask()
 fillTaskOnPage("all")
})
function deletTask(index){
 let task=tasksss[index]
  swal({
...
function storeTask(){
  let taskString=JSON.stringify(tasksss);
  localStorage.setItem("tasksss",taskString)
}
